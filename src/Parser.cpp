...
bool Parser::isEnd() const { return peek().kind == TokenKind::Eof; }
...
std::unique_ptr<TranslationUnit> Parser::parse() {
    ...
    while (!isEnd()) {
        auto d = parseDecl();
        if (d)
            trUnit->decls.push_back(std::move(d)); // typedef returns nothing
    }
    return trUnit;
}
...
std::unique_ptr<Type> Parser::parseBaseType() {
    ...
    default:
        ...
        auto t = take();
        if (auto it = aliases.find(t.data); it != aliases.end())
...
std::unique_ptr<Type> Parser::parseType() {
    ...
    if (peek().kind == TokenKind::LPar && (peek(1).kind == TokenKind::RPar || isType(peek(1)))) {
        take();
...
std::unique_ptr<Decl> Parser::parseDecl() {
    ...
    if (match(TokenKind::Typedef)) {
        ...
        userTypes.push_back(name.data);
        aliases[name.data] = std::move(t);
        return nullptr;
    }
    ...
    if (isType(peek())) {
        int i = 1;
        while (!isEnd() && peek(i).kind == TokenKind::Star)
            ++i;
        if (peek(i).kind != TokenKind::Identifier)
            fail("Expected identifier");
        if (peek(i + 1).kind == TokenKind::LPar)
...
std::unique_ptr<Decl> Parser::parseVarDecl() {
    ...
    if (leadConst) {
        if (auto p = dynamic_cast<PointerType*>(type.get())) {
            PointerType* in = p;
            while (auto next = dynamic_cast<PointerType*>(in->base.get()))
                in = next;
            in->constBase = true;
        } else
            isConst = true; // const int x
    }
    ...
    if (match(TokenKind::Assign)) {
        if (match(TokenKind::LBlock)) {
            // TODO nested {} for struct in struct
            while (!isEnd() && peek().kind != TokenKind::RBlock) {
                initList.push_back(parseExpr());
                if (!match(TokenKind::Comma))
                    break;
            }
            expect(TokenKind::RBlock);
    ...
    var->initList = std::move(initList);
    var->isConst = isConst;
    return var;
}
...
std::unique_ptr<Stmt> Parser::parseStmt() {
    ...
    case TokenKind::Semicolon: {
        take();
        s = std::make_unique<ExprStmt>();
        break;
    } // null stmt
    default:
        if (peek().kind == TokenKind::Const || isType(peek())) {
...
std::unique_ptr<Expr> Parser::parseExpr() { return parseBinary(); }
...
struct OpInfo {
    ...
    OpInfo(const Token& tok) {
        ...
        case TokenKind::PercAssign:
        case TokenKind::AmperAssign:
        case TokenKind::PipeAssign:
        case TokenKind::CarretAssign:
        case TokenKind::LessLessAssign:
        case TokenKind::GreatGreatAssign:
            prec = 0;
            rightAcc = true;
            break;
...
std::unique_ptr<Expr> Parser::parsePrimary() {
    ...
    case TokenKind::Typeid: {
        take();
        expect(TokenKind::LPar);
        auto node = std::make_unique<TypeidExpr>();
        node->offset = tok.offset;
        node->expr = parseExpr();
        expect(TokenKind::RPar);
        return node;
