...
int main(int argc, char *argv[]){
    ...
    for(int i = 1; i < argc; ++i){
        std::string a = argv[i];
        if(a == "-o"){ if(++i == argc) return usage(std::cerr); out = argv[i]; }
        else if(a == "-S") stage = Stage::Assembly;
        else if(a == "-c") stage = Stage::Object;
        else if(a == "-E") stage = Stage::Preprocess;
        else if(a == "--dump-tokens") dumpTokens = true;
        else if(a == "--dump-ast")    dumpAst = true;
        else if(a == "-h" || a == "--help"){ usage(std::cout); return 0; }
        else if(!a.empty() && a[0] == '-'){
            std::cerr << "comp: error: unknown option '" << a << "'\n";
            return usage(std::cerr);
        }
        else if(in.empty()) in = a;
        else { std::cerr << "comp: error: only one input file is supported\n"; return 1; }
    }
    if(in.empty()){ std::cerr << "comp: error: no input file\n"; return usage(std::cerr); }

    fs::path stem = fs::path(in).stem();   // basename without extension
    // default output name per stage, mirroring gcc
    if(out.empty()){
        switch(stage){
            case Stage::Link:       out = "a.out";                       break;
            case Stage::Assembly:   out = stem.string() + ".asm";        break;
            case Stage::Object:     out = stem.string() + ".o";          break;
            case Stage::Preprocess: out = "";                            break;  // stdout
        }
    }
    ...
    auto cleanup = [&]{ std::error_code ec; for(auto& p : temps) fs::remove(p, ec); };
    ...
    try{
        ...
        if(stage == Stage::Preprocess){
            if(out.empty()) std::cout << s;
            else { std::ofstream f(out); f << s; }
            return 0;
        }
        ...
        if(stage == Stage::Assembly) return 0;
        ...
        if(stage == Stage::Object){ cleanup(); return 0; }
