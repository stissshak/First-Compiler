...
struct SourceMap {
  public:
    void add(std::size_t offset, std::string file_name, std::size_t line) {
        fragments.push_back(SourceFragment{offset, std::move(file_name), line});
    }

    SourceLoc resolve(std::size_t offset, const std::string& buffer) const {
        auto it = std::upper_bound(
            fragments.begin(), fragments.end(), offset,
            [](std::size_t o, const SourceFragment& sf) { return o < sf.buffer_offset; });
        if (it == fragments.begin())
            return SourceLoc{"unknown", 1, 1};
        --it;
